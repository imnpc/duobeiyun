...
class DuobeiyunApi {
	 ...
	 * @var string
	 */
	public static $ROOM_TYPE_SMALL = "4";

    /**
	 * 房间类型小班客户端
     * @var string
     */
    public static $ROOM_TYPE_SMALL_CLIENT = "8";
	...
	public static $DEVICE_TYPE_PC = "1";

	public static $DEVICE_TYPE_MOBILE = "2";
	 ...
	 * @return string
	 */
	public function updateRoomScheduleByMinute($roomId, $startTime, $length) {
		$startTimeStr = date("Y-m-d H:i", $startTime);
		$params = array();
		$params["roomId"] = $roomId;
		$params["startTime"] = $startTimeStr;
...
//		$params["slidesFile"] = "@" . $filename;
		$params["slidesFile"] = new \CURLFile($filename);
		if (class_exists('\CURLFile')) {
			$params["slidesFile"] = new \CURLFile(realpath($filename));
		} else {
			$params["slidesFile"] = "@" . realpath($filename);
		}
	...
	public function generateRoomEnterUrl($uid, $nickname, $roomId, $userRole, $deviceType) {
		$params = array();
		$params["uid"] = $uid;
		...
		$url = $this->serverAddress . "/api/v3/room/enter?";
		$result = $url . implode("&", $queryStrings);
		return $result;
	}

    public function generateAuthInfoUrl($uid, $nickname, $roomId, $userRole) {
        $params = array();
        $params["uid"] = $uid;
        $params["nickname"] = $nickname;
        $params["roomId"] = $roomId;
        ...
        $url = $this->serverAddress . "/api/v4/room/authinfo?";
        $result = $url . implode("&", $queryStrings);
        return $result;
    }
	...
	//获取房间文档列表
	public function getDocumentListByRoomIdV4($roomIds) {
		$params = array();
		$params = $this->prepareParameters($params);
...
//		echo "\n";
	...
	private function buildParamStr($params) {
		$validParams = array();
		foreach($params as $key => $value) {
			//if(!empty($value) && $value != "") {
			if($value != null) {
	...
	private function postBody($path, $postFields) {
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $this->serverAddress . $path);
		curl_setopt($ch,CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_HEADER, false);
